...
export function create(level, top, left, parent, lineAmount, lineBool, idAddition){
    ...
    if (level == 0){
        ...
        calc.calcDecorator(sync.syncDecorator("variableSpellLevel", formfield.singleLine("variableSpellLevel"+idAddition,12,2,14,26, "level"+level+"SpellBarAsset"+idAddition, undefined, "center", undefined, undefined)))
        for (let i=0; i<4; i++){
...
export function create3_5(level, top, left, parent, lineAmount, lineBool, idAddition){
    lineBool = (lineBool === undefined || lineBool === null) ? true : lineBool;
    idAddition = (idAddition === undefined || idAddition === null) ? "" : idAddition;
    if  (level == "variable"){
        let SpellBarAsset = svg.create("level"+level+"SpellBarAsset"+idAddition, top, left, 197, 50, "spellbar.svg", parent)
