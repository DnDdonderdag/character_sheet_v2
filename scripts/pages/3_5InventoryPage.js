...
export function create(top,left){
    ...
    //Armor
    frame.create("armorFrameInventoryPage", 315, 435, 190, 200, "inventoryPage", undefined, undefined, undefined, false)
    let bonusText = text.create("acBonusTextInventoryPage", "AC", 187, 22, 40, 8, "armorFrameInventoryPage", undefined, 8, undefined, "center")
    let armorTypeText = text.create("acTypeTextInventoryPage", "ARMOR TYPE", 187, 65, 69, 8, "armorFrameInventoryPage", undefined, 8, undefined, "center")
    let checkPenaltyText = text.create("checkPenaltyTextInventoryPage", "PENALTY", 187, 136, 41, 8, "armorFrameInventoryPage", undefined, 8, undefined, "center")
    for(let i=0; i<4; i++){
        ...
        sync.syncDecorator("armorTypeInventoryPage"+String(i), calc.calcDecorator(formfield.singleLine("armorTypeInventoryPage"+String(i), 7 + 45*i, 65, 69, 42, "armorFrameInventoryPage", undefined, "left", undefined, undefined)))
        sync.syncDecorator("checkPenaltyInventoryPage"+String(i), calc.calcDecorator(formfield.singleLine("checkPenaltyInventoryPage"+String(i), 7 + 45*i, 135, 42, 42, "armorFrameInventoryPage", "#c5c6c7", "center", 25, undefined)))
    ...
    //Attunements
    let itemSlots = ["head", "neck", "shoulders", "eyes", "torso", "over torso", "waist", "arms", "hands", "ring", "ring", "feet"]
    frame.create("equippedItemsFrameInventoryPage", 735, 5, 620, 155, "inventoryPage", "EQUIPPED ITEMS", undefined, undefined, false)
    for(let j=0; j<4; j++){
        for(let i=0; i<3; i++){
            sync.syncDecorator("equippedItemInventoryPage"+String(i)+","+String(j), calc.calcDecorator(formfield.singleLine("equippedItemInventoryPage"+String(i)+","+String(j), 17 + 45*i, 10+150*j, 148, 32, "equippedItemsFrameInventoryPage", undefined, "left", undefined, undefined)))
            let line = document.createElement("div")
            line.id = "equippedLineAsset"+String(i)+","+String(j)
            line.className = "spellLineAsset"
            line.style.setProperty("top", 49 + 45*i +"px")
            line.style.setProperty("width", "148px")
            line.style.setProperty("left", 10+150*j+"px")
            document.getElementById("equippedItemsFrameInventoryPage").appendChild(line)
            text.create("equippedItemText"+i+"&"+j, itemSlots[j*3+i].toUpperCase(), 7 + 45*i, 10 + 150*j, 150, 10, "equippedItemsFrameInventoryPage", undefined, undefined, undefined, "left")
        }
    }

    //Treasures
    frame.create("treasureFrameInventoryPage", 525, 5, 410, 200, "inventoryPage", "TREASURES", undefined, undefined, false)
    sync.syncDecorator("treasures1", calc.calcDecorator(formfield.create("treasures1Inventory", 7, 10, 192, 343-165, "treasureFrameInventoryPage", undefined, undefined, undefined, undefined)))
    sync.syncDecorator("treasures2", calc.calcDecorator(formfield.create("treasures2Inventory", 7, 208, 192, 343-165, "treasureFrameInventoryPage", undefined, undefined, undefined, undefined)))
